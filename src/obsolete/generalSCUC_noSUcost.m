...
function [Pg,objective,onoff,y,demandVector]=generalSCUC(str,params,dynamicUCParams)
...
if(dynamicUC)
    ...
    if(dynamicUCParams.enforceOnoff && ~isempty(dynamicUCParams.originalOnoff))
...
if(dynamicUC && ~isempty(dynamicUCParams.originalPg))
    deviationPenalty=1;
...
%     w=zeros(nb,1); w((bus(:,PD)>1))=0; w=zeros(nb,1);
    currTime = mod(startTime-1+k-1,24)+1;
    w = params.windScenario(:,currTime);
    demandFactor=getDeterministicDemandFactor(currTime);
    bus=scale_load(demandFactor,mpc.bus); %mpc bus stays fixed
    ...
    for i_branch = 1:N_contingencies+1
        ...
        Constraints = [Constraints,Va(refs,k,i_branch)== bus(refs,VA)*(pi/180)]; %constrain ref angle to be
...
for unit = 1:ng
    initialOnOffLength=abs(params.initialState(unit));
    if(params.initialState(unit)>0)
    ...
    for t=initialOnOffLength+1:length(onExtended)
        ...
        Constraints = [Constraints, onExtended(t)+offExtended(t)<=1];
        Constraints = [Constraints, sum(squeeze(z(unit,t-initialOnOffLength,:)))==onExtended(t)];
        Objective=Objective+squeeze(z(unit,t-initialOnOffLength,:))'*suBlocksCosts;
        for j=1:min(J-1,t-1)
...
% ops = sdpsettings('solver','Cplex','cplex.epgap','1e-2','verbose',params.verbose); %gurobi,sdpa,mosek
%ops = sdpsettings('solver','Cplex');


result=optimize(Constraints,Objective,ops); %note that value(objective) is
% result=optimize(Constraints,Objective); %note that value(objective) is
%not the value of the objective for some reason, proper use will just be to
%calculate the objective cost given the solution (for OPF:
%sum(totcost(gencost,value(Pg))) )
Pg=value(Pg)*baseMVA;
