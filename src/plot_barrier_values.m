...
for j=1:i_CE-1
    for i = 1:length(S_sorted)
        if(~isempty(bestPlanVecTemp{K,i,j}))
            ...
            o4 = [o4,5*success_rate_barrier(bestPlanVecTemp{8,i,j},barrier_struct,params.alpha,i_CE-1)];
        end
    end
    values(:,j) = mean([o1;o2;o3;o1+o2;o1+o4],2);
end
figure(10);
titles={'planValues','success rate barrier values','success rate values','overall objective values','overall objective values - normalized barrier values'};
figure;
for i_plot=1:5
    subplot(2,3,i_plot);
    plot(values(i_plot,:));
