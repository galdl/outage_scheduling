function [planValues,success_rate_values,monthlyCost,contingenciesFrequency,planValuesVec,lostLoad,relative_nn_std_values,monthly_success_rate_values,...
    monthly_lost_load,monthlyCost_DA,monthly_lost_load_DA,relative_lostLoad_vector_means] = extractObjectiveValue(iterationDir,N_plans,params,config)
...
relative_lostLoad_vector_means = zeros(params.nb,N_plans);
for i_plan=1:N_plans
    try
        ...
        if(numOfMonthsPerPlan(i_plan)<round(0.75*params.numOfMonths)) %if not almost all months finished, dont use that data
            ...
            for i_matFile=1:length(monthFileList)
                ...
                for day = 1:length(monthlyStats)
                    ...
                    relative_lostLoad_vector = [relative_lostLoad_vector,monthlyStats{day}.relative_lostLoad_vector];
        ...
        %this is currently averaged across all months
        relative_lostLoad_vector_means(:,i_plan) = mean(relative_lostLoad_vector,2);
    catch ME
